<?php
    ...
    class User{
        ...
        function getUserEmail($email){
            $sql = $this->conexion->prepare("SELECT id_user, users.first_name, users.last_name, users.email, users.password, id_role FROM `user_role` INNER JOIN users ON users.id = user_role.id_user INNER JOIN roles ON roles.id = user_role.id_role WHERE users.email = ?"); 
